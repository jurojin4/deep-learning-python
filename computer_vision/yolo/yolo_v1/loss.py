...
class YOLOV1Loss(nn.Module):
    ...
    YOLOV1 Loss class.
    """
    def __init__(self, C: int, S: int = 7, B: int = 2, lambda_noobj: float = 0.5, lambda_coord: float = 5., reduction: Literal["mean", "sum"] = "sum") -> None:
        """
        ...
        :param float **lambda_noobj**: Weight for boxes coordinates. Set to `5.`.
        :param Literal["mean", "sum"] **reduction**: Set to `sum`.
        """
        assert isinstance(C, int), f"C has to be an {int} instance, not {type(C)}."
        ...
        assert isinstance(lambda_coord, float), f"lambda_coord has to be an {float} instance, not {type(lambda_coord)}."
